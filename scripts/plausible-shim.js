/* plausible-shim.js — keeps Plausible's class-based CTA tagging working
 * against the new tracker. Load AFTER the tracking snippet:
 *   <script defer src="https://a.example.com/js/script.js" data-key="ak_…"></script>
 *   <script defer src="/scripts/plausible-shim.js"></script>
 *
 *   <a class="plausible-event-name--signup_cloud" href="…">
...
(function () {
  ...
  // class names cannot contain one. `name` is the event, everything else a prop.
  function parse(el) {
    var name = null, props = {};
  ...
  function handle(e) {
    ...
    // tagged form wrapping its submit button counts twice.
  ...
  // Manual `plausible(...)` calls, for markup that mixes both styles.
